...
>
> **Onde estamos (atualizado em 2026-07-13, sessão 25):** o backend (`../back`) está com as Fases 1–12 concluídas, testado (113 testes, 99% cobertura) e **já deployado em produção no Railway**: `https://squadup-api.up.railway.app` (documentado em 2026-07-08 — ver §2). O front (`../front`) tem o protótipo visual 100% pronto (Fases 1–11 concluídas, Fase 12 em 6/8) e **já consome a API real para autenticação, partidas e chat da partida** — a Fase 13 (integração real) está em 13/16: fundação (13.1–13.3), Auth real (13.4), Matches reais (13.5, sessão 24) e Mensagens reais (13.6, sessão 25) concluídas. Restam avaliações (13.7), denúncias (13.8) e o fechamento ponta a ponta (13.9) — os dois Contexts restantes (`RatingsContext`, `ReportsContext`) ainda são 100% mockados. O `TCC.tex` está escrito como um **anteprojeto** (tempo futuro, "será implementado"), não como a monografia final — falta reescrever boa parte para refletir o que foi de fato construído, e os assets (prints, bibliografia) hoje só existem no Overleaf, fora do repositório.
...
2. **13.4–13.8 (independentes entre si, podem ser paralelas ou em qualquer ordem):** Auth real, Matches reais, Mensagens reais, Avaliações reais, Denúncias reais. **Auth real ✅ concluída** (sessões 22–23), **Matches reais ✅ concluída** (sessão 24, branch `feat/matches-real`) e **Mensagens reais ✅ concluída** (sessão 25, branch `feat/messages-real`) — restam Avaliações (13.7) e Denúncias (13.8).
