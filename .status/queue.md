...
- **Sessão 21 (2026-07-08):** Itens 3 e 4 da fila concluídos (13.2).
  - Item 3 — `src/services/api/client.ts`: wrapper de `fetch` tipado (`apiClient.get/post/patch/delete`), parse de erro no formato `{ detail: { code, message } }` do backend (classe `ApiError extends Error`, com fallback `UNKNOWN_ERROR` se o corpo não seguir o contrato), e anexação automática de `Authorization: Bearer` via `setAuthToken`/`getAuthToken` (módulo com token em memória — a persistência via `expo-secure-store` fica para o item 5, que vai chamar `setAuthToken` no boot). Base URL lida de `process.env.EXPO_PUBLIC_API_URL` (fallback `http://localhost:8000`); criado `.env.example` na raiz (`.env` real já estava no `.gitignore`, nunca existiu no repo). 8 testes em `src/services/api/__tests__/client.test.ts` (mock de `globalThis.fetch`).
  - Item 4 — `src/services/adapters/`: lido `../back/app/schemas/*.py` diretamente (não só o resumo do `backend-contract.md`) para garantir paridade exata de campo. `types.ts` define as interfaces `Api*` em `snake_case` espelhando os schemas Pydantic reais (`ApiPublicUser`, `ApiMyProfile`, `ApiParticipant`, `ApiMatchSummary`/`ApiMatchDetail`, `ApiMatchRef`, `ApiRating`, `ApiReport`, `ApiMessage`). Funções puras `toPublicUser`/`toMyProfile`/`toMatchSummary`/`toMatchDetail`/`toParticipant`/`toMatchRef`/`toRating`/`toReport`/`toMessage` fazem a conversão; `toRatingPayload` achata `RatingCriteria` para o `POST` de avaliação. Decisões registradas no código: `average_rating: null` (usuário sem avaliações) cai para `0` em `toPublicUser` — comportamento provisório, tipo `PublicUser.averageRating` continua `number`, tratamento definitivo de nulo é escopo da 13.7; horário do backend vem com segundos (`"09:00:00"`) e é cortado para `"09:00"` em `toMatchSummary` para casar com o formato usado nos mocks/telas. `Message.createdAt`/`Rating.createdAt`/`Report.createdAt` recebem o ISO completo devolvido pelo servidor sem reformatar — `MessageBubble` hoje imprime `createdAt` bruto (formato `"HH:mm"` do mock) e vai precisar passar a formatar a data na 13.6, quando `MessagesContext` parar de gerar timestamp no cliente (resolve D12); não alterado nesta sessão para não misturar escopo de adapter com escopo de tela. 17 testes novos em `src/services/adapters/__tests__/` (um arquivo por entidade).
  - Item 5 — instalado `expo-secure-store` (`npx expo install`, registrou o config plugin em `app.json`) e criado `src/services/storage/tokenStorage.ts`: `saveTokens`/`getAccessToken`/`getRefreshToken`/`clearTokens`/`restoreAuthToken`. Decisão registrada no código: `expo-secure-store` é um no-op em `react-native-web` (keychain/keystore não existem no browser, `ExpoSecureStore.web.js` exporta um objeto vazio) — no web o módulo cai para `sessionStorage` (limpa ao fechar a aba, aceitável para a demo acadêmica); no nativo (Expo Go/EAS) usa o keychain/keystore de verdade via `SecureStore`. `saveTokens`/`clearTokens`/`restoreAuthToken` já sincronizam o token em memória do `client.ts` (`setAuthToken`) — a `AuthContext` da 13.4 só vai precisar chamar essas funções nos momentos certos (login/refresh/logout/boot), sem repetir a lógica de storage. 7 testes novos (`tokenStorage.test.ts` para o branch nativo mockando `expo-secure-store`, `tokenStorage.web.test.ts` para o branch web mutando `Platform.OS` e mockando `sessionStorage`). Sub-fase 13.2 (itens 3–5) concluída.
  - Item 6 — instalado `@tanstack/react-query@^5` e criado `src/services/queryClient.ts` (instância única com `retry: 1`/`staleTime: 60s` de default) e `src/services/queryKeys.ts` (convenção central de chaves — `me`, `matches(filters?)`, `match(id)`, `messages(matchId)`, `userRatings(userId)`, `reports` — para as sub-fases 13.4–13.8 usarem sem reinventar o formato cada uma). `App.tsx` ganhou `<QueryClientProvider client={queryClient}>` por fora de `SafeAreaProvider`/todos os Contexts existentes (nenhum deles usa React Query ainda — só prepara o terreno). Teste novo `__tests__/App.test.tsx` (smoke: renderiza a árvore inteira sem quebrar; precisou mockar `global.css`, que o Jest não consegue parsear) e `src/services/__tests__/queryKeys.test.ts`. `npx expo export --platform web` confirmado ainda de pé após a mudança no componente raiz. Suíte total 218/218, lint e `tsc --noEmit` zerados. Sub-fase 13.3 concluída. Próxima tarefa: item 7 (campo de idade no cadastro, D15 — abre a 13.4, auth real).
...
**Concluídas:** 75 (69 do protótipo + refinamento visual transversal + itens 1–6 da Fase 13, sessões 20–21)
**Em andamento:** 0
**A fazer:** 11 (Fase 12: 12.3 requer dispositivo/emulador do usuário, 12.8 build de apresentação; Fase 13: itens 7–16)
