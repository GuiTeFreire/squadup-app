...
| D4 | Line endings CRLF | ~~Baixa~~ **Resolvida (sessão 35)** | Adicionado `.editorconfig` (`end_of_line = lf`, `insert_final_newline`, `trim_trailing_whitespace`, `indent_size = 2`) na raiz do projeto — solução definitiva mencionada aqui desde a descoberta original; `npm run lint:fix` deixa de ser a única linha de defesa. |
...
| D13 | Selo de verificado sem texto alternativo para leitor de tela | ~~Baixa~~ **Resolvida (sessão 35)** | Adicionado `accessibilityLabel="Verificado"` ao ícone `check-decagram` nos 6 arquivos citados (`ParticipantList.tsx`, `MatchDetailScreen.tsx`, `PublicProfileScreen.tsx`, `MyProfileScreen.tsx`, `RateUserScreen.tsx`, `PostMatchRatingScreen.tsx`). `TrustBadges.tsx` não precisou de mudança — já tem o texto "Verificado" visível ao lado do ícone. |
...
| D24 | Regra "avaliador precisa ter participado da partida" não documentada em nenhuma tela | ~~Baixa~~ **Resolvida (sessão 35)** | `useSubmitRating` (`src/hooks/useRatings.ts`) passou a repassar o `error` recebido do `onError` do `useMutation` para o callback do chamador (antes descartava o valor). `RateUserScreen.handleSubmit` agora detecta `error instanceof ApiError && error.code === "NOT_MATCH_PARTICIPANT"` e mostra uma mensagem específica ("Você e este participante precisam ter entrado na partida...") em vez do fallback genérico. Teste novo em `useRatings.test.tsx` confirma o passthrough do `code`. |
...
| D26 | `CreateMatchScreen` não dá feedback visual quando a localização não pôde ser capturada | ~~Baixa~~ **Resolvida (sessão 35)** | `CreateMatchScreen` ganhou o estado `locationChecked` (marcado `true` quando a checagem inicial de `requestLocation()` termina) e exibe o texto discreto "Localização não disponível — partida será criada sem coordenadas" logo abaixo do campo Local quando a checagem terminou e nenhuma coordenada foi obtida (`permissionDenied` ou falha de GPS) — sem bloquear o fluxo de criação, conforme D-Geo-3. 2 testes novos em `CreateMatchScreen.test.tsx`. |
...
| D34 | `validateEmail` (`RegisterScreen`/`LoginScreen`) é bem mais permissiva que o `EmailStr` do backend | ~~Baixa~~ **Resolvida (sessão 35)** | Extraída a validação duplicada para `src/utils/validation.ts` (`validateEmail`), com regex que exige domínio com TLD (`/^[^\s@]+@[^\s@]+\.[^\s@]+$/`), rejeitando casos como `"a@b"` que antes passavam no front e eram rejeitados só no backend. `RegisterScreen`/`LoginScreen` migrados para importar a função em vez de manter a cópia local `email.includes("@")`. 4 testes novos em `utils/__tests__/validation.test.ts`. |
...
- **Checkpointer — Sessão 35 (2026-07-30, branch `dev`):** limpeza de dívidas técnicas de baixa
  prioridade, a pedido do usuário ("quero tratar as dívidas técnicas uma a uma"), enquanto o item
  8 da Fase 14 (build `fa25bd21`) segue aguardando teste em dispositivo físico. Resolvidas: **D4**
  (`.editorconfig` com `end_of_line = lf`) · **D13** (`accessibilityLabel="Verificado"` no ícone
  `check-decagram` em 6 arquivos) · **D26** (aviso discreto em `CreateMatchScreen` quando a
  localização não é capturada) · **D34** (`validateEmail` extraída para `src/utils/validation.ts`,
  regex agora exige domínio com TLD, alinhada ao `EmailStr` do backend) · **D24**
  (`useSubmitRating` repassa o `ApiError` para o callback `onError`; `RateUserScreen` mostra
  mensagem específica para `NOT_MATCH_PARTICIPANT`). Não mexidas por não serem acionáveis por
  código agora (dependem de upgrade futuro, endpoint novo no backend, ou decisão de escopo da
  demo): D2, D3, D7, D10, D11, D22, D23, D25, D27. **309/309 testes** (7 novos: `validation.test.ts`,
  1 em `useRatings.test.tsx`, 2 em `CreateMatchScreen.test.tsx`), `tsc`/lint zerados. Nenhuma build
  EAS nova é necessária só por causa desta sessão (mudanças são de UI/validação, sem tocar em
  dependências nativas) — mas a build `fa25bd21` já em teste não inclui esses ajustes; se o
  usuário quiser vê-los no dispositivo, entra na próxima build gerada.
